...
class LoanDataPreprocessor:
    ...
    def transform(self, df, is_training=True):
        ...
        for col in numeric_cols:
            if col in df.columns:
                ...
                if missing_count > 0:
                    ...
                    print(f"   ✓ {col}: Filled {missing_count} missing values with {self.fill_values[col]}")
        ...
        df['Income_Loan_Ratio'] = df['TotalIncome'] / (df['LoanAmount'] + 1)  # +1 to avoid division by zero
        ...
        for col in encode_cols:
            if col in df.columns and col in self.label_encoders:
                ...
                df[col + '_Encoded'] = df[col].map(lambda x: mapping.get(x, default_value))
