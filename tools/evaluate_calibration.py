...
def build_model_input(df, expected, build_func=None):
    ...
    for feat in expected:
        if feat in df.columns:
            ...
            enc = f"{feat}_Encoded"
            if (df[feat].dtype == object or df[feat].dtype == 'O') and enc in df.columns:
                result[feat] = df[enc]
            else:
                result[feat] = df[feat]
        else:
            ...
            if feat == 'Total_Income' and 'TotalIncome' in df.columns:
                ...
                if feat == 'Property_Semiurban':
                    result[feat] = (df.get('Property_Area') == 'Semiurban').astype(int) if 'Property_Area' in df.columns else 0
                else:
                    result[feat] = (df.get('Property_Area') == 'Urban').astype(int) if 'Property_Area' in df.columns else 0
            else:
                result[feat] = 0
    return result
...
def main():
    ...
    for name_key, path in MODEL_FILES.items():
        ...
        if model_is_pipeline:
            ...
            expected = list(fnames) if fnames is not None and len(fnames) > 0 else comp.get('feature_names', []) or []
            X = build_model_input(df, expected)

        row = comp.get('models', {}).get(model_name_display, {})
