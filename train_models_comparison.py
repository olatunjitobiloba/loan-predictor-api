...
# LabelEncoder not needed in current comparison flow
...
def load_and_preprocess_data():
    ...
    try:
        ...
        # Fall back to separate train/test CSVs if the combined file is unavailable
        print("[DEBUG] Could not find loan_data.csv, falling back to train/test CSVs.")
        ...
        if os.path.exists(train_path):
            ...
            raise FileNotFoundError("Both train and test files are missing.")
    ...
    df = pd.get_dummies(
        df, columns=["Property_Area"], prefix="Property", drop_first=True
    )
    ...
    if not unlabeled_X.empty:
        ...
        print("✅ Saved processed test features: models/test_features_processed.csv")
        print(f"   Rows: {unlabeled_X.shape[0]}")
...
def main():
    ...
    for i, (model_name, row) in enumerate(comparison_df.iterrows(), 1):
        print(
            f"{i}. {model_name:25s} - {row['accuracy']:.4f} ({row['accuracy']*100:.2f}%)"
        )
