...
import sys
sys.path.insert(0, 'c:\\Dev\\loan-predictor-api')

from app_v7 import preprocess_input, load_model, model, feature_names
import joblib
...
def test_prediction():
    ...
    for test in test_cases:
        ...
        try:
            ...
            print(f"  Probabilities: [Rejected: {proba[0]:.4f}, Approved: {proba[1]:.4f}]")
            
            # Check if it's not always the same (the original bug)
            if pred != 0:  # If any prediction is not "Rejected"
                print(f"  Result: PASS (Model can predict approvals)")
            else:
                print(f"  Result: WARN (Model predicting rejection)")
